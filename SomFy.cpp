...
void SomFy::init(){
    ...
    if(debug){
        _serial->begin(38400);
        _serial->print("Simulated remote number : "); _serial->println(_remoteAdd, HEX);
        _serial->print("Current rolling code    : "); _serial->println(rollingCode);
    }
}
...
byte* SomFy::prepPacket(uint8_t btn){
    ...
    if(debug){
        ...
        for(byte i = 0; i < 7; i++) {
            ...
            _serial->print(payload[i],HEX); _serial->print(" ");
        }

        _serial->println(""); _serial->print("With checksum : ");
        for(byte i = 0; i < 7; i++) {
            if(payload[i] >> 4 == 0) {
                _serial->print("0");
                }
        ...
        _serial->println(""); _serial->print("Obfuscated    : ");
        for(byte i = 0; i < 7; i++) {
            if(payload[i] >> 4 == 0) {
                _serial->print("0");
            }
        ...
        _serial->println("");
        _serial->print("Rolling Code  : "); _serial->println(rollingCode);
    }
